...
| 5.2b Sécurité MCP HTTP | Serveur MCP en HTTP monté avec REST (`server.mcp.http`), clé dans `Authorization`, en-tête `Origin` validé, écoute sur `localhost` | N5, #39 |
...
| Accès | Exécution |
|---|---|
| Python | Un exemple par phase, sur la config `examples/j5/relance/` : `clients.py` (5.1a), `quotas.py` (5.1b), `securite.py` (5.2a) ; puis `tenant=…` sur le stockage Postgres |
