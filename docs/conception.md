...
security: {...}
server: {...}
profiles: {prod: {...}}               # surcharges partielles, fusionnées au chargement
```
...
**Réalisation (phase 5.5a) :** `profile` et `profiles` sont débloqués — `LATER_ROOT` se vide.

- **Trois états, pas deux.** Sans `profile`, loom se comporte **exactement** comme avant : les avertissements restent des avertissements et les deux erreurs restent des erreurs. `prod` durcit les premiers, `dev` assouplit les secondes. Aucune config existante ne change de comportement en s'installant.
- **Le profil se choisit à trois endroits**, du plus fort au plus faible : `--profile`, `LOOM_PROFILE`, puis `profile:` dans le fichier. `chosen_profile` rend l'arbitrage **et sa provenance**, que `loom validate` et `loom serve` impriment : un profil qu'on ne voit pas est un profil qu'on oublie. Un profil inconnu est refusé en nommant d'où il vient.
- **Ce que `prod` durcit** (une erreur de chargement là où c'était un avertissement) : juge corrélé et juge bloquant échantillonné (3.3), modèle sans tarif sous un budget en dollars (#010), secours à fenêtre plus petite (#10), service éparpillé dont les fichiers ne suivent pas (5.3c), API REST sans clé déclarée (#39), et `judges="skip"` (qui retire un contrôle).
- **Ce que `dev` assouplit** : un agent qui peut se mettre en pause sur un journal non durable (#28) — perdre un run en pause à la fermeture est le prix d'un essai sur une machine, pas en service.
- **`when.profiles` d'un juge** est débloqué : ailleurs, le juge est **sauté et le journal le dit** (`guard.checked`, `outcome: skipped`, `reason: other_profile`), plutôt que de ne pas être monté. Un juge déclaré ne disparaît pas en silence parce qu'on a changé de profil ; et c'est cohérent avec `when.tenants`, évalué lui aussi à l'exécution.
- **Un seul endroit décide** : `runtime.announce(config, warnings)` logue ou lève selon le profil. Les fonctions qui *trouvent* les avertissements n'ont pas changé — elles rendent toujours une liste de phrases.
...
- Chaque agent devient un outil MCP ; trois outils de contrôle s'ajoutent : `run_status`, `run_report` et `cancel`.
...
- **Ressources (5.4b, #32) :** le journal en lecture seule, sous `loom://`. Deux index se **listent** — `loom://runs` (la page de `Loom.runs()`, 5.4a) et `loom://sessions` — et le reste se construit d'un **gabarit**, puisqu'on ne peut pas énumérer les runs d'un client sans ouvrir chaque journal : `loom://runs/{run_id}{?session_id}` et `…/events`, `loom://sessions/{session_id}` et `…/events`, `loom://artifacts/{client}/{session}/{fichier}`. La session est un paramètre et non un segment, exactement comme en REST. Un fichier se lit aussi sous l'URI que le journal publie (`artifact://…`) : un `resource_link` rendu par loom n'est plus un lien mort, et c'est la réponse au manque relevé en 2.5 — les octets d'un fichier du run ne se téléchargeaient nulle part. Ils sont bornés par `execution.attachments.max_bytes`, faute d'une borne de lecture à part. Les règles sont celles des routes de lecture : `read` pour lire, masquage sans `read_content`, et `loom://sessions` refusé à une clé limitée à certains agents, là où `loom://runs` se filtre honnêtement. Le serveur déclare `subscribe: false` — pas d'abonnement, la limite « progression seulement » n'a pas changé.
- **Arrêt (5.4b, A5) :** outil `cancel` (`run_id`, `session_id`, `by`). Arrêter un run qu'on peut lancer n'est pas une permission de plus : c'est la portée `run` sur **son** agent, vérifiée après avoir lu de quel agent il est, comme en REST. Un run déjà terminé n'est pas une erreur — `cancelled: false`, et le texte le dit. Faute d'un `by`, c'est l'identifiant de la clé d'API qui signe la décision au journal ; en stdio il n'y a personne à nommer.

### 18.4 CLI

Dans le noyau, avec `argparse`. Commandes mentionnées dans la conception : `loom serve` (extra `http`, `--reload` en dev), `loom worker`, `loom mcp`, et les commandes de la fonction N4 : lancer un run, rejouer, inspecter une trace, valider la config. Réalisées : `validate`, `run`, `resume`, `serve`, `mcp`, `keys create`, `schema`, `report` (consommation d'un run ou d'une session, 3.4), `sessions list | export | delete` (F7, 4.1a), `approve` et `reject` (4.5) — la décision met un travail de reprise en file dans l'instance de la commande, qui la pilote et affiche la réponse ; `--no-wait` écrit et sort. Depuis 5.1a, `--tenant` dit au nom de quel client agir (`run`, `resume`, `approve`, `reject`, `mcp`), et `validate` montre les clients avec ce que chacun surcharge, puis monte les agents **par client**. Depuis 5.1b, `report --periode jour|mois` rend la consommation d'un client sur la fenêtre en cours — dépense, plafonds, reste et remise à zéro —, `validate` montre aussi le budget et le quota de chaque client, et `--tenant` s'applique enfin à `report` et aux trois `sessions`. Depuis 5.2a, `keys create` prend `--tenant`, `--expires` (date ISO ou durée : `90j`, `12h`) et `--rate-limit`, et imprime le bloc YAML complet ; `validate` montre une ligne par clé — client, portées, agents, débit, état de l'expiration — et signale une clé qui approuve sans pouvoir lire. Depuis 5.4b, `validate` et `serve` annoncent les ressources MCP quand le serveur HTTP est monté : elles ne dépendent d'aucun réglage, et c'est la clé de la requête qui dit ce qu'une lecture en voit. Depuis 5.5a, `--profile` est une option **globale** (elle l'emporte sur `LOOM_PROFILE` et sur le fichier), et `validate` comme `serve` impriment le profil actif, sa provenance et ce qu'il change. Depuis 5.4c, `validate` liste les **portes** déclarées avec leur route, leur agent, leur session et leur en-tête de livraison — et signale celle qui n'en a pas, dont une relivraison rouvrirait un run ; `serve` imprime l'adresse de chaque porte, celle qu'on donne au planificateur de la plateforme.
