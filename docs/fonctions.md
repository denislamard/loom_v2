...
- **Modèle sans tarif sous budget en dollars (backlog #010) :** avertissement au montage (logs) ; **erreur en profil prod** depuis 5.5a. Un budget en tokens reste efficace.
...
- **Expiration.** `expires_in` vaut **24 h par défaut** : rien d'autre ne borne l'attente d'un humain — ni le délai de l'agent, qui ne compte que le temps de pilotage (A6), ni la reprise, qui ne fait que reconstater l'attente. `null` le retire, et c'est alors un choix explicite. L'effet par défaut étant `deny`, une demande qui expire ne déclenche rien : l'appel est refusé, le modèle l'apprend. `expire_at` fait foi : à la reprise, une demande dont la date est passée est expirée, qu'un travail différé ait tourné ou non. Ce travail (`expire_approval`) ne fait que ramener le run à l'échéance ; comme il dort, il ne retient ni `drain()` ni la fermeture, et s'il est abandonné la demande n'en est pas moins périmée. L'effet est `deny` par défaut (l'appel est refusé, le run continue) ou `fail` (`error_type: approval.expired`).
...
**Réalisation (phase 4.3b) :**

- **Un enfant qui attend arrête son parent.** Quand le run d'un sous-agent se met en pause, l'appel qui l'a lancé **reste en suspens** : pas de `tool.completed`, donc rien n'est conclu. Le parent passe en `WAITING_CHILD` et rend sa concession, comme un run en pause. Son délai ne court pas pendant ce temps : l'étape est terminée, et l'attente n'en fait pas partie.
- **La racine rend ce qu'il faut trancher.** `RunResult.pending_approvals` d'un run porte les demandes de **tout son arbre** : l'appelant n'a pas à savoir qu'un sous-agent existe pour savoir ce qu'on lui demande. `approve()` et `reject()` retrouvent une demande par son `call_id` où qu'elle soit dans l'arbre, écrivent la décision dans le run qui la porte, et remettent la **racine** en file.
- **Reprise par le haut.** La racine rejoue l'appel délégant (`resumed: true`), lequel **reprend** l'enfant là où il s'était arrêté — un seul `run.started` pour lui. C'est la même mécanique que la reprise après un plantage (#26) : elle marche donc aussi entre deux process.
- **Les échéances de l'arbre.** Le réveil différé se calcule sur les demandes de **tout l'arbre** : quand c'est un sous-agent qui attend, la racine n'a pas de demande à elle, et personne ne viendrait la réveiller.
- **Savoir quand repartir.** Parent et enfant partagent le journal de la session : le parent y relit l'état de l'enfant, et n'avance que si l'enfant le peut. Récursif, pour un enfant qui en attend lui-même un. Rien n'est écrit tant que la décision n'est pas là, et une reprise pour rien ne coûte qu'une lecture.

**Réalisation (phase 4.5) — les canaux :** l'API REST (`POST /v1/runs/{id}/approve` et `/reject`, portée `approve` ; l'identifiant de la clé signe la décision, qu'un `by` du corps remplace), la CLI (`loom approve`, `loom reject`), et en MCP l'elicitation quand le client la déclare — sinon le run est rendu en pause, avec ce qu'il attend, et quelqu'un tranche par un autre canal. Le détail est en #40.

**Où s'écrit une décision (4.5) :** dans le span de **la demande qu'elle tranche**, et non à la racine du run. Une décision prise hors de la boucle — REST, CLI, `approve()` — n'a pas d'étape à elle : la ranger à la racine la séparait de sa demande, alors qu'un approbateur en ligne les écrit d'un bloc. Les deux chemins se lisent donc pareil ; le `call_id` reste le fil qui relie les quatre événements d'un appel (demande, décision, `tool.called`, `tool.completed`), le span n'étant qu'un regroupement.
...
- **Déclarer :** `judge:` sur l'agent (réponse finale) ou sur un rôle : `{model, name?, criteria: [{name, rule, min_score, blocking}], context, when: {sample, condition, tenants, profiles}, repair: {max_attempts, tools}, on_failure, fallback_message, llm, timeout, on_error}`. Défauts : `min_score` 0,8, `blocking: true`, `sample` 1, `repair: {max_attempts: 1, tools: auto}`, `on_failure: fail`, `on_error: block`. Nom par défaut : `output` pour la réponse finale, le nom du rôle pour un rôle ; unique dans l'agent. `when.profiles` borne un juge à certains profils (5.5a) : ailleurs il est **sauté**, et le journal le dit (`reason: other_profile`) plutôt que de ne pas le monter. Pas de juge sur un outil Python ou MCP : il ne se répare pas. Contrôles au chargement : modèle déclaré, critères et contexte sans double, `fallback` avec son message, capacité `vision` si le juge reçoit les pièces jointes.
...
**Réalisation (phase 5.3b) :** la file `rabbitmq` (extra `rabbitmq`, `aio-pika`) et la commande `loom worker`.

- **Publier d'un côté, consommer de l'autre.** `storage.queue: {backend: rabbitmq, url_env: NOM}` — la config nomme la variable qui porte l'URL du courtier, jamais l'URL. Une instance ordinaire, `loom serve` comprise, ne fait plus que **mettre en file** ; ce sont les workers qui pilotent. Le piège est qu'une file servie sans worker derrière laisse tout attendre : `loom validate` le dit en clair sous la ligne `File`.
- **`loom worker`** prend les tâches une à une (`--jobs N` pour en mener plusieurs), après avoir **repris au démarrage** ce qui traînait pour chaque client (`recover()`) — un worker qui démarre est souvent celui qui remplace un mort, et ce qui restait en plan n'est dans aucune file, il est au journal. `SIGINT` et `SIGTERM` demandent l'arrêt : plus de tâche prise, celles en cours vont au bout.
- **Au moins une fois.** L'acquittement suit l'exécution : un worker tué n'acquitte pas, et le courtier redélivre. Une tâche qui **échoue** est acquittée quand même et journalisée — la rejouer sans fin serait pire ; un message illisible aussi, pour la même raison.
- **Le bail, et ce qu'il fallait ajouter.** Le travail redélivré arrive **tout de suite**, alors que le bail du mort court encore : le second pilote se fait refuser par la concession (#27). Sans rien de plus, le run attendrait la prochaine reprise au démarrage. Un travail qui bute sur un bail est donc **reposé pour l'après-bail** (délai = ce qui reste du bail, plus une marge), et c'est ainsi qu'un run passe d'un worker à l'autre sans que personne s'en occupe. Borné à vingt attentes, après quoi le run est laissé au journal.
- **Le délai, que RabbitMQ n'a pas.** Un travail différé va dans une file d'attente sans consommateur, avec une durée de vie par message ; à l'échéance, le courtier le laisse tomber dans la file de travail. Aucun plugin n'est requis. Le prix est connu : cette file d'attente est une file, donc un message à longue échéance retient ceux qui le suivent.
- **Ce que le courtier ne sait pas dire.** `state` rend toujours `unknown` et `cancel` toujours faux : un courtier ne suit pas ses messages et ne retire pas ce qui est publié. Le port l'avait prévu, et l'état d'un run se lit au journal. `submit(key=…)` ne dédoublonne plus non plus — la clé devient l'**identifiant** du travail, et ce qu'elle protégeait l'est ailleurs : la concession pour un run, l'idempotence pour un effet.

**Réalisation (phase 5.3c) :** le **bus des nouvelles d'écriture**, port et deux adaptateurs (`postgres`, `redis`), plus le magasin d'idempotence `redis`.

- **Le bus ne transporte pas les événements.** Une `Notice` dit « du neuf dans le journal de (client, session), des seq N à M », et qui s'y intéresse relit le journal. Trois raisons : un `NOTIFY` de Postgres est borné à 8 000 octets et un événement les dépasse souvent ; le contenu ne sort jamais du journal, donc la politique de lignes (5.3a) et le masquage (5.2a) continuent de s'appliquer ; et le journal reste la seule source de vérité — un bus qui perd un message fait perdre une notification, jamais un événement.
- **Une source de plus, pas un chemin de plus.** Le journal notifiant publie après chaque écriture et, s'il a un bus, suit celles des autres : il relit et remet aux **mêmes abonnés**. SSE, `Loom.stream()`, la progression MCP et la CLI n'ont pas changé d'une ligne — c'est ce que le module annonçait depuis 1.6.
- **Chacun garde sa position.** Un abonné qui a une position pour ce journal relit depuis elle, ce qui rattrape ce que le bus a laissé passer ; sans position, il part du début du lot annoncé, pour ne pas remonter toute la session. Un process qui n'a **aucun abonné** ne relit rien : il avance sa position et s'arrête là.
- **Personne ne s'écoute soi-même.** Une nouvelle porte la `source` qui l'a écrite — l'identité de l'instance, celle qui prend aussi ses concessions (#27) : ses propres événements sont déjà remis à ses abonnés, les relire les remettrait deux fois.
...
**Réalisation (phase 5.4c — le webhook, et la planification confiée à la plateforme) :**

- **Une porte déclarée, parce que celui qui appelle ne connaît pas l'API de loom.** Un planificateur, un CRM, une passerelle de paiement envoient **leur** charge. La config dit donc quoi en faire : `triggers:` nomme une porte, son agent et un **gabarit de message** (`{{ payload.… }}`, le moteur de #50, avec `{{ trigger }}` et `{{ delivery }}`), et la route est `POST /v1/hooks/{nom}`. Un alias mince de la route de lancement n'aurait rien apporté : c'est le gabarit qui fait le travail.
- **Le client vient de la clé, et une porte ne le nomme pas.** C'est la règle de #34, et l'y déroger aurait donné à une clé le moyen de lancer un run au nom d'un autre. La liste `agents` d'une clé borne du même coup ce qu'elle peut déclencher, puisqu'une porte nomme son agent — aucun réglage de plus.
- **Une seconde livraison de la même chose ne relance rien.** L'en-tête déclaré (`delivery_header`) devient le **`run_id`** : le second appel retrouve le run (200 et `repeated`, au lieu de 202) au lieu d'en ouvrir un autre. Rien à tenir à côté — l'identifiant de livraison *est* l'identifiant du run. Sans en-tête déclaré, deux réveils font deux runs : `loom validate` le dit de la porte, pour le dire avant la panne.
- **Ce qu'une livraison apporte est contrôlé à la porte.** L'identifiant de livraison finit en nom de journal, donc en nom de fichier : il est vérifié là, et non au moment d'écrire (422). Une charge illisible est refusée sans rien ouvrir ; un corps **vide** est légitime — un planificateur n'a rien à dire d'autre que « c'est l'heure ». Une variable absente du gabarit donne une chaîne vide : une charge incomplète fait un message plus pauvre, pas une livraison perdue. Un gabarit de **session** qui rend le vide, lui, est refusé : ce n'est pas un nom.
- **La porte est une facette du journal** (`run.started.trigger`), pas du contenu : une clé de supervision peut demander ce qu'une porte a lancé, ce que la recherche de 5.4a rend immédiat.
- **La planification n'est pas dans loom** : aucun cron, aucun adaptateur. C'est la plateforme qui sonne — Cloud Scheduler, un cron, un planificateur de CI savent tous faire un POST —, et un en-tête de livraison daté (`$(date +%F)`) rend la tournée idempotente. Le déclencheur « file de messages » de H6 reste ouvert : il demanderait ses propres réglages, et rien ne le réclame.

**Reste connu (5.4c) :** **aucune signature n'est vérifiée** — la porte est protégée par une clé d'API, pas par le HMAC d'un émetteur (`Stripe-Signature`, `X-Hub-Signature-256`) ; un service tiers qui ne sait envoyer qu'une signature ne peut donc pas sonner directement, il faut un relais qui porte la clé. La charge n'est **pas gardée** : seul le message rendu entre au journal, si bien qu'un gabarit qui oublie un champ le perd pour de bon (le nom de la porte, lui, est en facette). Une livraison déjà vue rend le run **quel que soit son état** : si le premier run a échoué, la relivraison ne le rejoue pas — elle constate. Le gabarit ne sait ni boucler ni conditionner (#50), donc une charge qui porte une liste d'éléments ouvre **un** run pour toute la liste, pas un par élément. Et la session d'une porte étant tirée de la charge, des livraisons du même sujet s'accumulent dans un même journal, qui grandit jusqu'à la compaction.
...
**Réalisation (phase 5.5b — le sceau ; #30) :** `storage.encryption: {keys: [NOM_DU_SECRET]}` scelle la **charge** de chaque événement et les **octets** de chaque fichier, en AES-256-GCM.

- **Ce qui est fermé, ce qui reste ouvert.** La charge part en un seul bloc ; l'enveloppe reste en clair — identifiants, horodatage, type, catégorie, statut, agent, rôle, facettes, `seq`. C'est ce qui fait qu'un journal scellé **se filtre comme un journal en clair** : les colonnes et les facettes `jsonb` des stores SQL sont remplies avec ces champs, et une facette est par règle un champ qui ne porte pas de contenu (5.4c). Le nom d'un outil se cherche donc encore dans un journal scellé ; ses arguments, non.
- **La clé vient des secrets, jamais de la config.** `keys` nomme des secrets, et chaque client redirige ce nom vers sa variable (`secrets: {LOOM_JOURNAL_KEY: MARTIN_JOURNAL_KEY}`) : le mécanisme de 5.1a, sans rien de neuf. Sa règle joue ici à plein — une redirection vers une variable **absente** vaut vide, jamais le secret commun. Effacer la variable d'un client scelle son journal pour de bon sans toucher à celui du voisin, et c'est exactement le *crypto-shredding*. Sans redirection, deux clients lisent la même variable : le chargement le **dit**, car effacer cette clé les effacerait ensemble.
- **Plusieurs clés, dans l'ordre : la première ferme, toutes ouvrent.** C'est le renouvellement. Chaque sceau porte l'**empreinte** de la clé qui l'a fermé (condensé tronqué, jamais la clé), si bien qu'un journal mêle sans peine de l'ancien et du neuf, et qu'une clé absente est **nommée** au lieu d'être devinée.
- **Sans la clé, un journal se voit et s'efface, mais ne se lit pas.** Lister les sessions et connaître la longueur d'un journal ne lisent que la **marque** d'une ligne (session, rang, date) : ce que la suppression RGPD exige — voir ce qu'il reste, le retirer — n'a pas besoin du contenu. Relire, replier un run, rejouer : refusés, en nommant le client et l'empreinte manquante. En REST, c'est un **424** — la demande est juste, ce qui manque est ailleurs.
- **Un sceau ne se déplace pas.** Ce que l'AAD authentifie sans le chiffrer : le client, la session, le rang et l'identifiant de l'événement — pour un fichier, son URI entière. Le type, le statut et les facettes n'y sont pas : l'enveloppe les recopie de la charge et les **vérifie à chaque relecture**, donc une charge remise sous un autre type est déjà une erreur de validation. L'AAD couvre ce que personne ne vérifie : la place.
- **Le sceau vit dans les stores, pas dans un enrobage du port.** Le port `EventStore` parle `Event`, et un `Event` ne peut pas porter une charge fermée : l'union des charges est discriminée par `type`, et tout lecteur de charge — moteur, projections, rejeu, masquage — devrait apprendre à la narrer. Le sceau est donc un **codec** de ligne, là où la sérialisation vit déjà ; les fichiers, eux, sont bien un enrobage d'`ArtifactStore`, puisque ce port parle d'octets.
- **Une ligne en clair reste lisible.** Déclarer le sceau ne réécrit rien : l'ancien se relit tel quel, ce qui s'ajoute est fermé. L'inverse n'est pas vrai, et c'est le principe.

**Reste connu (5.5b) :** un **magasin d'idempotence partagé** (`sqlite`, `postgres`, `redis`) garde le résultat d'un outil (`IdempotencyRecord.result`) et n'est pas scellé — son contenu vit le temps d'un TTL, mais il vit en clair ; sa clé métier peut en plus porter une référence client. Le magasin `journal`, lui, range ses enregistrements **dans** le journal, donc sous le sceau : c'est le seul qui suive, et il ne convient qu'aux clés techniques d'un appel.

**Deuxième reste connu, vu au run réel de 5.5b : le nom d'un fichier est le SHA-256 de son contenu en clair.** L'URI d'un artefact est adressée par le contenu (`artifact_uri`), calculée par le noyau **avant** le sceau : deux exécutions qui produisent les mêmes octets donnent le même nom de fichier, quelles que soient les clés — c'est ce que le run réel a montré, en retrouvant le nom du run simulé. Le sceau cache donc le contenu, mais qui a accès au dossier peut **confirmer** un contenu qu'il devine (il en calcule le condensé et cherche le nom), et reconnaître le même fichier chez deux clients. Le corriger demanderait de nommer un fichier par un condensé **à clé** (HMAC de la clé du client), ce qui touche le noyau, change les URI déjà écrites dans les journaux et fait perdre l'adressage par contenu — à décider, pas à rattraper d'un correctif. Le sceau ne protège pas non plus les exports faits par un accès (`loom sessions export` écrit en clair, avec la clé), ce qui est voulu. Une clé effacée est un avertissement au chargement dans **tous** les profils, prod compris : la config ne peut pas distinguer un effacement voulu d'une variable oubliée, et refuser de démarrer arrêterait le service des autres clients. Un seul algorithme, sans choix : AES-256-GCM, clé de 32 octets en base64.

**Réalisation (phase 5.5c — la rétention ; #30) :** `storage.retention: {events_days: N}` efface les sessions dormantes, et `tenants[].retention` donne sa borne à un client.

- **L'unité est la session, et la borne se lit sur sa dernière écriture.** Un journal privé de ses premiers événements ne se replie plus et ne se rejoue plus : il n'y a rien d'honnête à retirer *dans* une session. Une session dont la dernière écriture dépasse la borne part donc en entier — journal, fichiers, clés d'idempotence —, par le même chemin que la suppression RGPD demandée à la main (`Loom.delete_session`).
- **Aucun contenu n'est lu, et c'est ce qui rend la rétention possible après un effacement de clé.** La décision tient dans la **marque** d'une session (client, session, rang, date), que les quatre journaux donnent sans ouvrir une charge : un journal scellé dont la clé a disparu (5.5b) s'efface donc comme les autres — là justement où la place serait perdue pour de bon. C'est aussi pour cela que le sceau tient l'horodatage depuis 5.5c : un champ dont dépend une suppression doit être authentifié.
- **Conséquence assumée :** la rétention ne sait pas qu'un run n'a jamais fini. Une session muette plus longtemps que la borne part, même si un run y attendait une approbation — une borne plus courte que le délai d'une approbation détruit des approbations en attente.
- **Rien ne s'efface tout seul, et l'essai à blanc est le défaut.** `loom retention` dit ce qui partirait ; `--yes` supprime. Aucun cron dans loom (comme les portes de 5.4c) : c'est la plateforme qui met la commande à l'heure. Le rapport dit son **prix** (sessions regardées, gardées) et la borne appliquée à chaque client, parce qu'une règle qui n'efface rien et une règle non déclarée se ressemblent trop. En essai à blanc, les fichiers et les clés d'une session restent **inconnus** (`null`) au lieu d'être comptés à zéro : ils ne se comptent qu'en supprimant.
- **`events_days` est un entier positif :** zéro voudrait dire « efface tout, maintenant », y compris la session qu'un run est en train d'écrire. Un client annule la règle commune avec `retention: {events_days: null}`, ce qui s'écrit noir sur blanc.

**Reste connu (5.5c) :** effacer la session d'un run **en train de tourner** (ou en pause, qu'on reprendrait ensuite) laisse le moteur écrire dans un journal disparu, et l'échec n'est pas propre — une erreur de projection dans une tâche de fond. La borne d'au moins un jour met la plupart des runs hors d'atteinte, mais pas un run en pause depuis plus longtemps que la borne ; s'en prémunir demanderait de lire le journal, donc la clé. La rétention ne borne que les **sessions** : rien n'est prévu pour élaguer le passé d'une session vivante (la compaction, elle, réduit l'historique du modèle sans rien effacer), et rien ne purge un magasin d'idempotence partagé en dehors des sessions qu'il accompagne.
...
**Réalisation (phase 5.4b — les mêmes lectures, en ressources MCP) :**

- **Un outil fait travailler, une ressource se lit.** Ce que 5.4a rend par des routes se lit sous `loom://` : deux index **listés** — `loom://runs` (la page de `Loom.runs()`) et `loom://sessions` — et cinq **gabarits** pour le reste, puisqu'énumérer les runs d'un client demanderait d'ouvrir chaque journal. La session est un **paramètre** (`loom://runs/{run_id}{?session_id}`) et non un segment, comme en REST : un run appartient à une session, ou porte son propre identifiant.
- **Les octets d'un fichier se lisent enfin** (`loom://artifacts/{client}/{session}/{fichier}`), et aussi sous l'URI que le journal publie (`artifact://…`) — un `resource_link` rendu par loom n'est plus un lien mort. C'est la réponse au manque relevé en 2.5 ; la borne est `execution.attachments.max_bytes`, faute d'une borne de lecture à part.
- **Les règles sont celles des lectures REST, et elles n'ont pas été réécrites** : `read` pour lire, masquage sans `read_content` (une ressource est une relecture), `loom://sessions` refusé à une clé limitée à certains agents et `loom://runs` filtré honnêtement, un client qui vient de la clé et d'elle seule. Une clé sans `read` ne voit **aucune** ressource : il n'y a rien à lui montrer.
- **Pas d'abonnement.** Le serveur déclare `subscribe: false` : la limite « progression seulement, pas de flux d'événements » tient, et le direct reste le SSE de REST. Déclarer les ressources suffit à le dire — c'est le SDK qui l'écrit dans les capacités.
- **`cancel`** complète les deux outils de contrôle. La portée `run` sur l'agent du run, lu d'abord pour le connaître ; un run déjà terminé rend `cancelled: false` sans être une erreur ; la clé d'API signe la décision au journal faute d'un `by`.

**Reste connu (5.4a) :** une page ouvre un journal par session, donc son coût suit le **nombre de sessions du client** et non la taille de la page — mesuré sur deux dossiers réels le 24/09 : 17 journaux ouverts pour rendre 3 runs sur l'un, 96 sur l'autre, où la borne par défaut arrête la recherche ; c'est ce nombre-là qui ramènera la projection le jour venu. `since` et `until` filtrent **après** le repli : ils ne font économiser aucune lecture. Le tri par dernière écriture porte sur ce que la recherche a déjà trouvé, si bien qu'une borne atteinte peut laisser dehors un run plus récent d'un journal non ouvert — la page le dit par `truncated`, elle ne le corrige pas. Les facettes libres ne sont pas interrogeables par l'URL. `GET /events` cherche sur tout le client quand aucune session n'est nommée, sans index autre que celui du magasin. Et une liste rendue à une clé limitée à certains agents est filtrée **après coup**, donc sa borne `limit` a compté des runs que l'appelant ne verra pas.

**Reste connu (5.4b) :** une ressource n'est pas paginable — `loom://runs` rend la page par défaut (50 runs, 50 journaux ouverts) et `loom://sessions` **tout** ce que le client a, sans borne ni curseur, là où `GET /events` en a un ; un client qui voudrait autre chose doit passer par REST. Les traces se lisent en entier (`…/events`), sans les critères d'une `EventQuery` : c'est la porte de la recherche qui manque du côté MCP. Les octets d'un fichier partent en base64 dans la réponse, donc dans le contexte du client, et un fichier plus gros que `execution.attachments.max_bytes` est refusé plutôt que découpé. Une clé sans `read` ne voit aucune ressource, mais les **outils**, eux, restent listés comme avant — la liste des outils ne filtre que par agent. Et `cancel` porte un nom hors de la famille des deux autres outils de contrôle (`run_status`, `run_report`), parce que #40 l'annonce ainsi depuis J1.
...
**Réalisation (phase 5.1b — ce qu'un client consomme ; J3, J4, L3) :**

- **Budget d'un client (`budgets.tenant`)** — `max_cost_per_day`, `max_tokens_per_day`, `max_cost_per_month`, `max_tokens_per_month`. Fenêtres **calendaires, en UTC**, et non glissantes : un compteur par fenêtre suffit alors, il se reconstruit depuis le journal, et la remise à zéro est une date qu'on peut annoncer à l'appelant. Il voyage avec les autres budgets — `budgets` de la racine, `budgets` de la fiche du client, fusionnés clé par clé.
- **Lu une fois, au lancement du run**, et non avant chaque appel comme le budget d'un run. Une enveloppe quotidienne épuisée ne veut pas dire « réponds vite » mais « reviens demain » : le run qui franchit le plafond va jusqu'au bout, et c'est le **suivant** qui est refusé (`BudgetExhausted` : la limite atteinte, ce qui a été dépensé, et les secondes jusqu'à la remise à zéro). Un run refusé **n'écrit rien** — il n'a pas eu lieu.
- **Compteur** — port `UsageCounter` (`consumed`, `record`) et adaptateur mémoire ; Postgres et Redis en 5.3. `record` **pose** ce qu'un run racine a coûté au lieu de l'ajouter : c'est ce qui le rend idempotent et permet au **réchauffage** — la relecture des `model.responded` du journal depuis le début de la fenêtre, une fois par client et par fenêtre — de chevaucher la vie courante sans compter deux fois. Seul un run **racine** est enregistré : la consommation d'un sous-agent est déjà dans le total de son parent.
- **Quota d'un client (`quotas.runs_per_minute`)** — fenêtre **glissante** de soixante secondes, vérifiée à la façade, donc valable par les quatre accès. Un plafond de débit qui laisserait passer le double à cheval sur la minute n'en serait pas un. Un sous-run n'est pas une demande du client : il est lancé par son parent.
- **Débit d'une clé d'API (`security.api_keys[].rate_limit`, #39)** — même fenêtre glissante, mais c'est la **clé** qu'on protège et non le client : elle compte toutes ses requêtes, lectures comprises, et deux clés d'un même client peuvent ne pas avoir le même débit. Vérifié par l'accès HTTP seul.
- **Ordre du contrôle, du moins cher au plus cher** : le client, l'agent qu'on lui ouvre, son débit (en mémoire), puis son budget de période (qui peut relire le journal). Rien n'est écrit tant que les quatre ne sont pas passés.
- **Rapport (J5)** — `Loom.consumption(tenant_id, period="day" | "month")` et `loom report --periode jour|mois [--tenant …]` : ce qui a été dépensé sur la fenêtre, ce qu'il en reste, le nombre de runs et la date de remise à zéro. Il **relit le journal** à chaque appel au lieu de lire le compteur : c'est une question qu'un humain pose, et elle doit valoir même pour un client sans budget — dont le compteur, justement, ne retient rien.
- **Accès** — en Python, `run()`, `stream()` et `submit()` lèvent `BudgetExhausted` ou `QuotaExceeded` ; en REST, **429** avec un `Retry-After` (quelques secondes pour un débit, la bascule de la fenêtre pour une journée épuisée) ; `loom validate` montre le budget et le quota de chaque client, et le `--tenant` qui manquait à `report` et à `sessions` en 5.1a est réparé.

**Reste connu (5.1b) :** le compteur vit dans le process — un second worker a le sien, qu'il réchauffe depuis le journal (compteur partagé : 5.3) ; des runs lancés **en parallèle** sont tous admis, le contrôle ayant lieu avant qu'aucun n'ait consommé ; la dépense d'une **compaction** n'entre au compteur qu'au réchauffage suivant, n'étant le résultat d'aucun run de l'appelant (le rapport, lui, la voit toujours) ; un run refusé ne laisse aucune trace au journal, seulement un avertissement dans les logs — c'est pourquoi `BudgetScope` et `budget.exceeded` restent ceux du run et de la session ; les fenêtres sont en **UTC** et non dans le fuseau du client ; pas de `max_calls` par période (les tokens disent la même chose) ; le quota compte les lancements, pas les appels de modèle.
...
**Réalisation (phase 5.5a — profils dev et prod ; M4) :**

- **Trois états, pas deux.** Sans `profile`, le comportement est **exactement** celui d'avant : les avertissements restent des avertissements, les erreurs des erreurs. `prod` durcit, `dev` assouplit. Aucune config existante ne change de sens le jour où les profils arrivent.
- **Le profil se choisit à trois endroits**, du plus fort au plus faible : `--profile`, `LOOM_PROFILE`, puis `profile:` dans le fichier — un même fichier sert donc en dev et en prod, ce qu'un conteneur sait poser. `loom validate` et `loom serve` impriment le profil actif **et sa provenance** : un profil qu'on ne voit pas est un profil qu'on oublie. Un profil inconnu est refusé en nommant d'où il vient.
- **Les surcharges (`profiles: {prod: {…}}`) fusionnent au chargement**, avant tous les contrôles : les objets en profondeur, les **listes remplacées**, `params` et `llm` remplacés en bloc. Une liste à demi fusionnée ne voudrait rien dire — que ferait-on d'un troisième modèle à moitié surchargé ?
- **Ce que `prod` durcit** : juge corrélé, juge bloquant échantillonné, modèle sans tarif sous un budget en dollars, secours à fenêtre plus petite, service éparpillé dont les fichiers ne suivent pas, API REST sans clé déclarée, et `judges="skip"`.
- **Ce que `dev` assouplit** : un agent qui peut se mettre en pause sur un journal non durable.
- **Un seul endroit décide** : `runtime.announce(config, warnings)` logue ou lève selon le profil. Les fonctions qui trouvent les avertissements n'ont pas bougé — elles rendent toujours des phrases.

**Reste connu (5.5a) :** les surcharges d'un profil portent sur le **fichier racine** ; les agents, lus depuis `agents_dir`, n'en reçoivent aucune (`agents` est une liste, donc remplaçable en bloc seulement). Il n'y a que deux profils : pas de `staging`, pas de profil nommé librement. `prod` ne durcit que ce que quelqu'un a pensé à lui donner — la liste ci-dessus est fermée, et un nouvel avertissement doit être branché à `announce` pour en faire partie. Le profil est lu au **chargement** : changer `LOOM_PROFILE` ne change rien à une instance déjà construite. Et `judges="skip"` refusé en prod l'est à la façade, donc pour les quatre accès — la portée `admin` de l'accès REST ne suffit plus à le rattraper.
...
**Réalisation (phase 5.2a — clés complètes et contenu ; N3, §14.2) :**

- **Fin de validité (`expires`)** — une clé peut porter une date. Elle est alors **reconnue puis refusée** (401 « clé expirée le … ») : « expirée » se corrige, « inconnue » envoie chercher au mauvais endroit. Rien ne tombe au chargement — un service redémarré la nuit ne doit pas s'arrêter pour une clé qu'on avait laissée traîner —, et `loom validate` signale une clé périmée ou qui expire sous sept jours. Une rotation, en V2, c'est deux clés déclarées en même temps, l'ancienne portant sa date de fin ; une révocation, une clé retirée de la config.
- **`read_content` : ce qu'une lecture montre.** Chaque charge d'événement déclare ses `content_fields` — les champs qui portent ce qu'un utilisateur a écrit, ce qu'un modèle a répondu, ce qu'un outil a reçu et rendu. Sans la portée, les routes répondent quand même, mais privées de ces champs, et la liste de ce qui a été retiré est nommée à côté (`redacted`). Une supervision garde donc les statuts, les durées, les coûts, la ventilation et les notes des juges ; elle perd la correspondance, les arguments d'outils, les motifs de verdict, les noms de fichiers et les arguments d'une approbation en attente.
- **Les trente charges le déclarent, même vide**, et un essai l'exige : c'est ce qui empêche une charge nouvelle de porter du contenu que le masquage ignorerait. Un chemin peut descendre d'un cran (`context.metadata`, `criteria[].reason`), ce qui garde les notes d'un juge en retirant leur prose.
- **Le masquage rend un dictionnaire, pas un événement** : un champ obligatoire retiré ne repasserait pas la validation, et c'est voulu — un journal masqué n'est pas un journal qu'on peut rejouer. Le journal, lui, garde tout : c'est un réglage d'accès, pas de stockage (§14.2).
- **Ce qu'une clé lance, elle le reçoit** : la réponse d'un `POST …/runs` n'est jamais masquée. C'est la **relecture** qui demande la portée — `GET /runs/{id}`, le SSE d'un run, l'export JSONL d'une session, la fiche d'une session. Conséquence assumée : une clé qui lance en `background: true` et va chercher son résultat plus tard a besoin de `read_content`.
- **Approuver demande de lire.** Les arguments d'une demande en attente sont du contenu : une clé qui a `approve` sans `read_content` trancherait à l'aveugle. `loom keys create` et `loom validate` le disent tous les deux.
- **`loom keys create`** gagne `--tenant`, `--expires` (une date ISO ou une durée : `90j`, `12h`) et `--rate-limit`, et imprime le bloc YAML complet à coller. **`loom validate`** montre une ligne par clé : client, portées, agents, débit, état de l'expiration.
- **Portée des accès** : `read_content` et `rate_limit` sont des notions **REST**. Python, la ligne de commande et le MCP en stdio n'ont pas de clé, donc rien n'y est masqué — c'est le serveur MCP en HTTP (5.2b) qui apportera la clé au protocole.

**Reste connu (5.2a) :** un export masqué n'est plus rejouable, puisqu'il manque des champs obligatoires ; le masquage retire le champ entier — il ne coupe ni ne résume, et le masquage fin (e-mails, IBAN) reste à J6 avec `telemetry.redaction` ; un champ vide n'est pas retiré et n'est pas annoncé, pour ne pas faire croire qu'on cache quelque chose ; `by` (l'auteur d'une décision) et `user_id` restent visibles, étant de l'audit et non du contenu ; l'expiration est lue à l'identification, donc une requête déjà en cours n'est pas interrompue quand la clé expire ; rien ne limite le nombre de clés déclarées ni ne les fait tourner toutes seules.

**Réalisation (phase 5.2b — le serveur MCP en HTTP ; N5) :**

- **La clé arrive à chaque requête**, et c'est tout ce qui change. En stdio, rien dans le protocole ne dit au nom de qui on parle : un serveur sert **un** client, choisi à son lancement. En HTTP, un **seul serveur sert tous les clients** — il publie à chacun les agents que sa clé lui ouvre, et refuse le reste. La limite de 5.1a tombe.
- **L'authentification a lieu avant le protocole**, dans une couche ASGI : une clé absente, inconnue ou expirée reçoit un **401 ordinaire** avec son `WWW-Authenticate`, plutôt qu'une erreur d'outil que le LLM du client essaierait d'interpréter. L'appelant reconnu est posé dans une variable de contexte, que les gestionnaires relisent.
- **Portées et contenu, comme en REST** : lancer un agent demande `run`, relire (`run_status`, `run_report`) demande `read`, et une clé limitée à certains agents ne voit publiés que ceux-là. Sans `read_content`, `run_status` rend le statut et les coûts sans la correspondance — tandis que le résultat d'un run lancé par la requête n'est jamais masqué. Une seule règle pour les deux accès à clé, puisque c'est la même clé (`Caller`, partagé).
- **Monté dans l'application REST**, sous `<base_path>/mcp` : un port, une authentification, un déploiement. Le gestionnaire de session du SDK vit aussi longtemps que l'application.
- **Protection du transport (spec MCP)** : `Origin` absent passe — un client natif n'en envoie pas — ; déclaré il passe ; inconnu il tombe en **403**. `Host` doit toujours figurer dans la liste, et un hôte étranger donne **421**. Une liste d'hôtes vide ferait tout refuser : **loom la remplit lui-même** avec son adresse d'écoute, `server.mcp.allowed_hosts` ne servant qu'à l'étendre.
- **Sans clé déclarée, le MCP HTTP ne monte pas** : `server.mcp.http` exige `security.api_keys`, contrôlé au chargement. Le MCP publie des outils à un LLM tiers, sur le réseau — l'ouvrir sans authentification serait une faute de configuration, pas un mode d'usage. Le stdio, lui, reste sans clé : c'est le process qui l'a lancé qui décide.
- **`approve` n'est toujours pas un outil MCP**, transport ou pas : c'est la raison d'être de l'elicitation et de l'API REST.

**Reste connu (5.2b) :** **un second serveur MCP en HTTP dans le même process ne répond plus** — le premier échange passe, les suivants restent sans réponse (« ASGI callable returned without completing response »). Reproduit avec le SDK seul, sans une ligne de loom : ce n'est donc pas notre défaut, mais il borne les exemples et les essais à **un** serveur par process, ce qui est de toute façon la forme d'un déploiement. Par ailleurs : le chemin monté répond sur `<base_path>/mcp/`, un appel à `<base_path>/mcp` recevant une redirection que les clients suivent ; les ressources `loom://` et l'outil `cancel` sont arrivés en 5.4b ; l'authentification est lue à l'ouverture de la requête, donc une clé qui expire pendant un run ne l'interrompt pas ; le débit d'une clé (`rate_limit`) n'est vérifié que par les routes REST, pas par le transport MCP.
...
- **MCP :**
  - chaque agent devient un outil MCP, et trois outils de contrôle s'y ajoutent (`run_status`, `run_report`, `cancel`) ;
...
**Réalisation (phase 4.5) :**

- **Arrière-plan par REST :** `background: true` dans le corps de `POST /v1/agents/{name}/runs`. La réponse est 202 et `{run_id, session_id, status}` — pas le résultat. Le run est **inscrit au journal avant la réponse** (H5) : l'identifiant rendu désigne un run qui existe, qu'on peut lire, suivre en SSE ou arrêter aussitôt. Une seule route pour les deux modes : c'est la même demande, seule l'attente change.
- **Trancher par REST :** `POST /v1/runs/{id}/approve` et `/reject`, portée `approve` — la première fois qu'elle sert. Corps : `call_id` (sans lui, tout ce que le run attend est tranché), `reason`, et pour un accord `arguments` corrigés. La réponse rend les appels tranchés ; ce qu'il reste à trancher se relit sur le run (`pending_approvals`), pour n'avoir qu'une vérité à tenir.
- **Qui signe :** l'approbateur inscrit au journal est **l'identifiant de la clé d'API**, qu'un `by` dans le corps remplace. Une passerelle nomme ainsi l'humain qui a tranché ; une instance locale sans clé n'a personne à nommer, et l'audit tient à ce que l'appelant a bien voulu dire.
- **Arrêter par REST :** `POST /v1/runs/{id}/cancel`, portée `run` — arrêter un run qu'on peut lancer n'est pas une permission de plus. Rend `cancelled: false` si le run était déjà fini.
- **Sessions par REST :** `GET /v1/sessions` (liste), `GET /v1/sessions/{id}` (la fiche : les runs de la session et ce qui attend un humain, tous runs confondus), `GET /v1/sessions/{id}/events` (le journal en JSONL), `DELETE /v1/sessions/{id}` (RGPD). La liste est **refusée à une clé limitée à certains agents** : elle ne dit pas de quels agents sont les runs d'une session, et la filtrer honnêtement demanderait de lire chaque journal. La fiche et l'export vérifient le droit sur chaque agent rencontré ; l'effacement, irréversible et commun à tous les agents de la session, demande `admin`.
- **Fiche de session (Python) :** `Loom.session(session_id)` rend `SessionInfo` — taille du journal, date de la dernière écriture, un résumé par run (agent, statut, run délégant, itérations, coût) et les approbations en attente de la session entière. C'est la question qu'on pose après un lancement en arrière-plan : « où en est cette conversation, et qu'attend-elle de moi ? »
- **Elicitation MCP :** quand le client déclare la capacité, le serveur monte un **approbateur en ligne** (#28) bâti dessus : chaque demande part en formulaire (accorder ou refuser, avec un motif) et la réponse est tranchée dans la boucle. Le run ne passe pas par `PAUSED`, et le journal garde qui a décidé — `mcp:<client>`, faute d'identité plus précise sur stdio. Un formulaire MCP n'a qu'un niveau de propriétés : corriger les arguments d'un appel n'y entre pas, et reste à l'API.
- **Sinon, la pause rendue :** sans elicitation, l'outil de l'agent **rend la main dès que le run s'arrête**, et sans `isError` — un run qui attend un humain n'est pas un échec. Le texte dit ce qui attend et comment reprendre ; le résultat structuré porte `status: paused`, le `run_id` et `pending_approvals`, ajouté aux trois accès (3.6). Un humain tranche ailleurs (REST ou `loom approve`), et `run_status` relit le run.
- **`approve` n'est toujours pas un outil MCP** (#39) : le LLM du client validerait les effets qu'il demande lui-même. C'est la raison d'être des deux chemins ci-dessus.
- **CLI :** `loom approve <run_id>` et `loom reject <run_id>` (`--call`, `--by`, `--reason`, `--arguments` pour un accord, `--session`). La décision met un travail de reprise en file **dans l'instance de la commande** : par défaut c'est donc ce terminal qui mène le run à son terme et affiche la réponse ; `--no-wait` écrit et sort, la reprise revenant à qui écoute ailleurs. `loom run` liste en fin de sortie ce qu'un run en pause attend, avec la commande qui le trancherait.
